...
mod __test__ {
  ...
  fn test_large_string_interning() {
    ...
    assert_eq!(interner.get_target(target_id).unwrap().as_ref(), large_string);
    assert_eq!(interner.get_message(message_id).unwrap().as_ref(), large_string);
  ...
  fn test_unicode_string_interning() {
    ...
    for unicode_str in &unicode_strings {
      ...
      assert_eq!(interner.get_target(target_id).unwrap().as_ref(), *unicode_str);
      assert_eq!(interner.get_message(message_id).unwrap().as_ref(), *unicode_str);
